...
RSpec.describe QueueUnmatchedImagesJob, type: :job do
  context 'queue unmatched images' do
    ...
    it 'should enqueue image matching jobs' do
      expect do
        image.save
      end.to change(ActiveJob::Base.queue_adapter.enqueued_jobs, :count).by(0) # same as above
    end
  end
  ...
  context 'queue match image jobs' do
    ...
    it 'should enqueue image matching jobs' do
      ...
      end.to change(ActiveJob::Base.queue_adapter.enqueued_jobs, :count).by(10) # from 11 to 10, while jobs are disabled above
    end
  end
end
